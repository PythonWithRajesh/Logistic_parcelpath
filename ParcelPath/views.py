...
def track_parcel(request):
    ...
    progress_val = 20
    active_stage = 1
    tracking_id = request.POST.get("tracking_id") or request.GET.get("tracking_id")

    if tracking_id:
        tracking_id = tracking_id.strip()
        try:
            parcel = Book_parcel.objects.get(tracking_id__iexact=tracking_id)
            if parcel.status == 'Delivered':
                progress_val = 100
                active_stage = 5
            elif parcel.status == 'Out for Delivery':
                progress_val = 85
                active_stage = 4
            elif parcel.status == 'In Transit':
                progress_val = 65
                active_stage = 3
            elif parcel.status == 'Picked Up':
                progress_val = 40
                active_stage = 2
            ...
            messages.error(request, "❌ Invalid Tracking ID! Please check your ID and try again.")

    return render(request, "track_parcel.html", {
        "parcel": parcel,
        "searched_id": tracking_id or "",
